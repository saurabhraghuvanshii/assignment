...
type RapidoPlaywrightTokenPayload = {
  userId: string;
  issuedAt: number;
  expiresAt: number;
};
...
export function createRapidoPlaywrightToken(
  userId: string,
  ttlMs: number = DEFAULT_TTL_MS,
): string {
  const now = Date.now();
  const payload: RapidoPlaywrightTokenPayload = {
    userId,
    issuedAt: now,
    expiresAt: now + ttlMs,
  };
...
export function verifyRapidoPlaywrightToken(
  token: string,
): RapidoPlaywrightTokenPayload {
  ...
  ) as RapidoPlaywrightTokenPayload;
