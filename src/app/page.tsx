    ...
    <div className="app-container">
        ...
        <div className="header-right">
          <div className="mode-switcher">
            <button
              ...
              onClick={() => { setAssistantMode('rides'); setActiveTab('dashboard'); }}
            >
              <Car size={14} />
              Rides
            </button>
            <button
              ...
              onClick={() => { setAssistantMode('food'); setActiveTab('dashboard'); }}
            >
          ...
          <div className="nav-tabs">
            ...
            <button className={`nav-tab ${activeTab === 'patterns' ? 'active' : ''}`} onClick={() => setActiveTab('patterns')}>
      ...
      {toast && (
        <div className={`toast ${toast.type}`}>
          {toast.type === 'success' ? <Check size={16} color="var(--accent-success)" /> : <X size={16} color="var(--accent-danger)" />}
          {toast.message}
        </div>
      )}
    </div>
...
function RideAssistant({ activeTab, showToast }: { activeTab: string; showToast: (msg: string, type: 'success' | 'error') => void }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch('/api/suggestions/confirm', {
        ...
        body: JSON.stringify({ suggestionId: suggestion.id, platform: selectedPlatform, rideType: selectedRideType }),
      });
      const data = await res.json();
      if (data.success) {
        setConfirmed(true);
    ...
    try {
      await fetch('/api/suggestions/edit', {
        ...
        body: JSON.stringify({
          ...
          editedFields: {
            origin: editOrigin !== suggestion.origin.address ? { address: editOrigin } : undefined,
            destination: editDest !== suggestion.destination.address ? { address: editDest } : undefined,
            departureTime: editTime !== suggestion.suggestedDepartureTimeDisplay ? editTime : undefined,
  ...
  if (activeTab === 'history') {
    return (
      <div>
        <div className="section-title"><History size={18} /> Ride History</div>
        ...
        {historyLoading ? (
          <div className="loading-container"><div className="loading-spinner" /><div className="loading-text">Loading rides...</div></div>
        ) : (
          <>
            ...
            <div className="pagination">
              <button className="page-btn" disabled={historyPage <= 1} onClick={() => setHistoryPage((p) => p - 1)}>← Previous</button>
              <span className="page-info">Page {historyPage} of {historyTotal}</span>
              <button className="page-btn" disabled={historyPage >= historyTotal} onClick={() => setHistoryPage((p) => p + 1)}>Next →</button>
            </div>
          </>
        )}
      </div>
  ...
  if (activeTab === 'patterns') {
    return (
      <div>
        <div className="section-title"><Brain size={18} /> Learned Ride Patterns</div>
        {patternsLoading ? (
          <div className="loading-container"><div className="loading-spinner" /><div className="loading-text">Loading patterns...</div></div>
        ) : (
          <div className="patterns-grid">
            {patterns.map((pattern) => (
              <div key={pattern.id} className="pattern-card">
                <div className="pattern-header">
                  <div className="pattern-day">{dayNames[pattern.dayOfWeek]} at {formatHour(pattern.hourOfDay)}</div>
                  <div className="confidence-bar"><div className="confidence-fill" style={{ width: `${pattern.confidence * 100}%` }} /></div>
                </div>
            ...
            {patterns.length === 0 && (
              <div className="no-suggestion">
                <div className="no-suggestion-title">No patterns detected yet</div>
                <div className="no-suggestion-text">Patterns will appear after the learning engine analyzes your ride history.</div>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div>
      {loading ? (
        <div className="loading-container"><div className="loading-spinner" /><div className="loading-text">Analyzing your ride patterns...</div></div>
      ) : hasSuggestion && suggestion ? (
        <div className={`suggestion-card ${confirmed ? 'confirmed' : ''}`}>
          <div className="suggestion-header">
            <div className={`suggestion-badge ${confirmed ? 'confirmed-badge' : ''}`}>
              {confirmed ? (<><Check size={12} /> Confirmed</>) : (<><Zap size={12} /> Proactive Suggestion</>)}
            </div>
            {!confirmed && (<button className="dismiss-btn" onClick={handleDismiss} title="Dismiss"><X size={16} /></button>)}
          </div>
          {confirmed ? (
            <div className="confirmed-overlay">
              <div className="confirmed-icon"><Check size={32} color="#0a0a0f" /></div>
              <div className="confirmed-text">Ride Confirmed!</div>
              <div className="confirmed-subtext">
                {selectedPlatform ? `${selectedPlatform.charAt(0).toUpperCase() + selectedPlatform.slice(1)}` : 'Your ride'} to{' '}
                {shortAddress(suggestion.destination.address)} • Departing at {suggestion.suggestedDepartureTimeDisplay}
              </div>
            </div>
          ) : editing ? (
            <div className="edit-overlay">
              <div className="edit-field">
                ...
                <input className="edit-input" value={editOrigin} onChange={(e) => setEditOrigin(e.target.value)} />
              </div>
              <div className="edit-field">
                <div className="edit-label">Destination</div>
                <input className="edit-input" value={editDest} onChange={(e) => setEditDest(e.target.value)} />
              </div>
              <div className="edit-field">
                <div className="edit-label">Departure Time</div>
                <input className="edit-input" value={editTime} onChange={(e) => setEditTime(e.target.value)} />
              </div>
              <div className="edit-field">
                <div className="edit-label">Platform</div>
                <select className="edit-select" value={selectedPlatform || ''} onChange={(e) => setSelectedPlatform(e.target.value)}>
                  <option value="uber">Uber</option><option value="ola">Ola</option><option value="rapido">Rapido</option>
                </select>
              </div>
              <div className="edit-field">
                <div className="edit-label">Ride Type</div>
                <select className="edit-select" value={selectedRideType || ''} onChange={(e) => setSelectedRideType(e.target.value)}>
                  <option value="cab">Cab</option><option value="auto">Auto</option><option value="bike">Bike</option><option value="premium">Premium</option>
                </select>
              </div>
            ...
            <>
              <div className="explanation-text">
                <AlertTriangle size={14} style={{ marginRight: 6, verticalAlign: 'middle', color: 'var(--accent-warning)' }} />
                {suggestion.explanation}
              </div>
              <div className="route-display">
                <div className="route-point">
                  <div className="route-label"><MapPin size={10} style={{ marginRight: 4, verticalAlign: 'middle' }} /> Pickup</div>
                  <div className="route-address">{shortAddress(suggestion.origin.address)}</div>
                </div>
                <div className="route-arrow"><div className="route-arrow-line" /><ArrowRight size={14} /></div>
                <div className="route-point">
                  <div className="route-label"><MapPin size={10} style={{ marginRight: 4, verticalAlign: 'middle' }} /> Drop-off</div>
                  <div className="route-address">{shortAddress(suggestion.destination.address)}</div>
                </div>
              </div>
              <div className="departure-time-display">
                <Clock size={18} style={{ color: 'var(--accent-primary-light)' }} />
                <div>
                  <div className="departure-label">Suggested Departure</div>
                  <div className="departure-value">{suggestion.suggestedDepartureTimeDisplay}</div>
                </div>
                <div style={{ marginLeft: 'auto' }}>
                  <span className={`traffic-badge traffic-${suggestion.liveData.trafficCondition}`}>
                    <TrendingUp size={10} />
                    {suggestion.liveData.trafficCondition === 'low' ? 'Clear' : suggestion.liveData.trafficCondition === 'moderate' ? 'Moderate' : suggestion.liveData.trafficCondition === 'heavy' ? 'Heavy' : 'Severe'}
                    {suggestion.liveData.trafficDelayMinutes > 0 && ` +${suggestion.liveData.trafficDelayMinutes}m`}
                  </span>
                </div>
              </div>
              <div className="platforms-grid">
                {groupedQuotes.map((q, i) => (
                  <div key={`${q.platform}-${q.rideType}-${i}`}
                    ...
                    onClick={() => { if (q.available) { setSelectedPlatform(q.platform); setSelectedRideType(q.rideType); } }}>
              ...
              <div className="action-buttons">
                <button className="btn-confirm" onClick={handleConfirm} disabled={confirming}>
                  {confirming ? (<><RefreshCw size={16} className="loading-spinner" style={{ border: 'none', width: 16, height: 16 }} /> Confirming...</>) : (<><Check size={16} /> Confirm Ride</>)}
                </button>
                <button className="btn-edit" onClick={handleEdit}><Pencil size={14} /> Edit</button>
              </div>
            </>
          )}
        </div>
      ...
      {!loading && (
        <div className="stats-bar">
          <div className="stat-item">
            <div className="stat-value">{suggestion?.confidence ? `${Math.round(suggestion.confidence * 100)}%` : '--'}</div>
            <div className="stat-label">Pattern Confidence</div>
          </div>
          <div className="stat-item">
            <div className="stat-value">{suggestion?.liveData?.trafficDelayMinutes || 0}m</div>
            <div className="stat-label">Traffic Delay</div>
          </div>
          <div className="stat-item">
            <div className="stat-value">{groupedQuotes.filter((q) => q.available).length || 0}</div>
            <div className="stat-label">Options Available</div>
          </div>
          <div className="stat-item">
            <div className="stat-value">
              {groupedQuotes.length > 0 ? `₹${Math.min(...groupedQuotes.filter((q) => q.available).map((q) => q.price))}` : '--'}
            </div>
...
function FoodAssistant({ activeTab, showToast }: { activeTab: string; showToast: (msg: string, type: 'success' | 'error') => void }) {
  ...
  const [editItems, setEditItems] = useState<Array<{ name: string; price: number; quantity: number }>>([]);
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch('/api/food/suggestions/confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ suggestionId: suggestion.id, platform: suggestion.platform }),
      });
      const data = await res.json();
      if (data.success) {
        setConfirmed(true);
  ...
  const updateEditItem = (index: number, field: 'name' | 'quantity', value: string | number) => {
  ...
  const handleSaveEdit = async () => {
    if (!suggestion) return;
    try {
      const itemsChanged = JSON.stringify(editItems) !== JSON.stringify(suggestion.items);
      await fetch('/api/food/suggestions/edit', {
        ...
        body: JSON.stringify({
          suggestionId: suggestion.id,
          editedFields: {
            restaurant: editRestaurant !== suggestion.restaurantName ? editRestaurant : undefined,
            platform: editPlatform !== suggestion.platform ? editPlatform : undefined,
  ...
  if (activeTab === 'history') {
    return (
      <div>
        <div className="section-title"><History size={18} /> Order History</div>
        <div className="filter-bar">
        ...
        {historyLoading ? (
          <div className="loading-container"><div className="loading-spinner" /><div className="loading-text">Loading orders...</div></div>
        ) : (
          <>
            <div className="ride-history-list">
              {orders.map((order) => (
                <div key={order.id} className="ride-item">
                  ...
                  <div className="ride-info">
                    ...
                    <div className="ride-meta">
                      ...
                      <span>{(order.items as Array<{ name: string }>).map((i) => i.name).join(', ')}</span>
                    </div>
                  </div>
                  <div className="ride-cost">₹{Math.round(order.totalCost)}</div>
                </div>
              ))}
            </div>
            <div className="pagination">
  ...
  if (activeTab === 'patterns') {
    return (
      <div>
        <div className="section-title"><Brain size={18} /> Learned Food Patterns</div>
        {patternsLoading ? (
          <div className="loading-container"><div className="loading-spinner" /><div className="loading-text">Loading patterns...</div></div>
        ) : (
          <div className="patterns-grid">
            {patterns.map((pattern) => (
              <div key={pattern.id} className="pattern-card food-pattern-card">
                <div className="pattern-header">
                  <div className="pattern-day">{dayNames[pattern.dayOfWeek]} at {formatHour(pattern.hourOfDay)}</div>
                  <div className="confidence-bar"><div className="confidence-fill food-confidence" style={{ width: `${pattern.confidence * 100}%` }} /></div>
                </div>
                <div className="pattern-route">
                  <ChefHat size={12} style={{ marginRight: 4, verticalAlign: 'middle' }} />
                ...
                <div className="food-pattern-items">
                  {(pattern.typicalItems || []).slice(0, 3).map((item, i) => (
                    <span key={i} className="food-item-chip">{item.name}</span>
                  ))}
                </div>
                <div className="pattern-details">
            ...
            {patterns.length === 0 && (
              <div className="no-suggestion">
                <div className="no-suggestion-title">No food patterns detected yet</div>
                <div className="no-suggestion-text">Patterns will appear after the learning engine analyzes your order history.</div>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div>
      {loading ? (
        <div className="loading-container"><div className="loading-spinner" /><div className="loading-text">Analyzing your food patterns...</div></div>
      ) : hasSuggestion && suggestion ? (
        <div className={`suggestion-card food-suggestion ${confirmed ? 'confirmed' : ''}`}>
          <div className="suggestion-header">
            <div className={`suggestion-badge food-badge ${confirmed ? 'confirmed-badge' : ''}`}>
              {confirmed ? (<><Check size={12} /> Confirmed</>) : (<><UtensilsCrossed size={12} /> Proactive Suggestion</>)}
            </div>
            {!confirmed && (<button className="dismiss-btn" onClick={handleDismiss} title="Dismiss"><X size={16} /></button>)}
          </div>
          ...
          {confirmed ? (
            <div className="confirmed-overlay">
              <div className="confirmed-icon food-confirmed-icon"><Check size={32} color="#0a0a0f" /></div>
              <div className="confirmed-text" style={{ color: 'var(--accent-food)' }}>Order Confirmed!</div>
              <div className="confirmed-subtext">
                {suggestion.restaurantName} via {suggestion.platform.charAt(0).toUpperCase() + suggestion.platform.slice(1)} •{' '}
            ...
            <div className="edit-overlay">
              <div className="edit-field">
                ...
                <input className="edit-input" value={editRestaurant} onChange={(e) => setEditRestaurant(e.target.value)} />
              </div>
              <div className="edit-field">
                <div className="edit-label">Platform</div>
                <select className="edit-select" value={editPlatform} onChange={(e) => setEditPlatform(e.target.value)}>
              ...
              <div className="edit-field">
                ...
                {editItems.map((item, i) => (
                  <div key={i} className="edit-item-row">
                    <input className="edit-input edit-item-name" value={item.name} onChange={(e) => updateEditItem(i, 'name', e.target.value)} placeholder="Item name" />
                    <select className="edit-select edit-item-qty" value={item.quantity} onChange={(e) => updateEditItem(i, 'quantity', parseInt(e.target.value))}>
                      {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}×</option>)}
                    </select>
                    <button className="edit-item-remove" onClick={() => removeEditItem(i)} title="Remove"><X size={14} /></button>
                  </div>
                ))}
              </div>
              <div className="edit-actions">
                <button className="btn-save-edit food-save" onClick={handleSaveEdit}>Save Changes</button>
                <button className="btn-cancel-edit" onClick={() => setEditing(false)}>Cancel</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="food-order-card">
                <div className="food-restaurant-header">
                  <div>
                    <div className="food-restaurant-name">{suggestion.restaurantName}</div>
                    <div className="food-restaurant-meta">
                      <span className="food-cuisine-tag">{suggestion.cuisine}</span>
                      <span className="food-platform-tag">{suggestion.platform === 'swiggy' ? 'Swiggy' : 'Zomato'}</span>
                    </div>
                  </div>
                  <div className="food-delivery-info">
                    ...
                    <div className={`delivery-badge delivery-${suggestion.liveData.deliveryCondition}`}>
                      <Truck size={10} />
                      {suggestion.liveData.deliveryCondition === 'normal' ? 'On Time' : suggestion.liveData.deliveryCondition === 'busy' ? 'Busy' : 'Very Busy'}
                      {suggestion.liveData.extraDelayMinutes > 0 && ` +${suggestion.liveData.extraDelayMinutes}m`}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="food-alternatives-grid">
                    {suggestion.alternatives.map((alt, i) => (
                      <div key={i} className="food-alt-card">
                        <div className="food-alt-name">{alt.restaurantName}</div>
                        <div className="food-alt-meta">
                          <span className="food-cuisine-tag small">{alt.cuisine}</span>
                          <span><Star size={10} /> {alt.rating}</span>
                        </div>
                        <div className="food-alt-details">
                          <span><Timer size={10} /> {alt.deliveryTimeMin} min</span>
                          <span>~₹{alt.estimatedCost}</span>
                          <span className="food-alt-platform">{alt.platformDisplayName}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Actions */}
              <div className="action-buttons">
                <button className="btn-confirm food-confirm" onClick={handleConfirm} disabled={confirming}>
                  {confirming ? (<><RefreshCw size={16} className="loading-spinner" style={{ border: 'none', width: 16, height: 16 }} /> Confirming...</>) : (<><Check size={16} /> Confirm Order</>)}
                </button>
                <button className="btn-edit" onClick={handleEdit}><Pencil size={14} /> Edit</button>
              </div>
            </>
      ...
      {!loading && (
        <div className="stats-bar">
          <div className="stat-item food-stat">
            <div className="stat-value food-stat-value">{suggestion?.confidence ? `${Math.round(suggestion.confidence * 100)}%` : '--'}</div>
            <div className="stat-label">Pattern Confidence</div>
          </div>
          <div className="stat-item food-stat">
            <div className="stat-value food-stat-value">{suggestion?.estimatedDeliveryMin || 0}m</div>
            <div className="stat-label">Est. Delivery</div>
          </div>
          <div className="stat-item food-stat">
            <div className="stat-value food-stat-value">{suggestion?.alternatives?.length || 0}</div>
            <div className="stat-label">Alternatives</div>
          </div>
          <div className="stat-item food-stat">
            <div className="stat-value food-stat-value">{suggestion ? `₹${suggestion.estimatedCost}` : '--'}</div>
